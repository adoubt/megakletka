...
func _init_poi() -> void:
	...
	for e_id in days_list:
		...
		chosen.clear()
		var mushroom_mult_size :float
		for poi_name in mushroom_pool:
			var position: Vector3 = _get_random_position_in_radius()
			var t = randf() # 0..1
			mushroom_mult_size = lerp(0.2, 10.0, pow(t, 2.5))
			pois_to_create.append({"day_id":day_id, "poi_name":poi_name, "position": position,"mushroom_mult_size":mushroom_mult_size})

			chosen.append(poi_name)
			if chosen.size() >= MUSHROOM_POI_ON_DAY:
				break		
		var merchant_pos: Vector3 = _get_random_position_in_radius(3.0, 7.0)
